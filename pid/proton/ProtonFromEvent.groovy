...
class ProtonFromEvent {
  ...
  if (event.dc1_status.contains(index)){
      ...
      def hit = event.dc1.get(index).find{ hit -> hit.layer == 6}
          if (hit){
              ...
              def (phi_DC_min, phi_DC_max) = DCRphimaxmin(theta_DC, sec, 1)
  ...
  if (event.dc1_status.contains(index)){
      ...
      def hit = event.dc2.get(index).find{ hit -> hit.layer == 18}
          if (hit){
              def (theta_DC, phi_DC) = thetaphifromhit([hit.x, hit.y, hit.z], sec)
              def (phi_DC_min, phi_DC_max) = DCRphimaxmin(theta_DC, sec, 2)      
              return phi_DC > phi_DC_min && phi_DC < phi_DC_max
          } else {
              return false
          }
  ...
  if (event.dc1_status.contains(index)){
      ...
      def hit = event.dc3.get(index).find{ hit -> hit.layer == 36}
          if (hit){
              def (theta_DC, phi_DC) = thetaphifromhit([hit.x, hit.y, hit.z], sec)
              def (phi_DC_min, phi_DC_max) = DCRphimaxmin(theta_DC, sec, 3)      
              return phi_DC > phi_DC_min && phi_DC < phi_DC_max
          } else {
              return false
          }
  ...
  def DCRphimaxmin(theta_DC, sec, region){

    def pid = 1
    def calc_phi_min = minparams[pid][sec][region-1][0] + minparams[pid][sec][region-1][1] * Math.log(theta_DC) 
                          + minparams[pid][sec][region-1][2] * theta_DC + minparams[pid][sec][region-1][3] * theta_DC * theta_DC;

    def calc_phi_max = maxparams[pid][sec][region-1][0] + maxparams[pid][sec][region-1][1] * Math.log(theta_DC) 
                          + maxparams[pid][sec][region-1][2] * theta_DC + maxparams[pid][sec][region-1][3] * theta_DC * theta_DC;
